...
- `website/` — trang giới thiệu tĩnh (`index.html` + `styles.css`), mở trực tiếp bằng trình duyệt

## Ảnh trên website hiện tại

`website/assets/` đang dùng 3 ảnh minh hoạ tải từ Wikimedia Commons (giấy phép rõ ràng, xem
`website/assets/CREDITS.md`) — KHÔNG phải ảnh sản phẩm/vườn thật của nhà mình. Khi có ảnh/video
thật (chụp mẻ sắn dây, vườn cam nhà mình), đưa vào `website/assets/`, xoá 3 file `*-stock.jpg`
...
2. **Link Facebook** — hiện đang tạm ẩn (comment) trong `website/index.html` và để
...
- Website chưa được deploy lên internet, mới chỉ là file tĩnh. Khi cần đưa lên mạng thật,
  có thể deploy miễn phí qua GitHub Pages hoặc Cloudflare Pages — nhờ Claude làm khi bạn sẵn sàng.
